...
{
    int n_vtex = g->n_vtex;
    int dist[n_vtex]; /* dist values used to pick minimum weight edge in cut */
    min_heap * q = malloc_min_heap(n_vtex); /* min priority queue */
    for(int i = 0; i < n_vtex; ++i)
    {
        dist[i] = ((unsigned)1 << 31) - 1;
        q->a[i] = malloc_min_heap_vtex(i, dist[i]);
        q->pos[i] = i;
    }
    dist[src] = 0; /* make src vertex extracted first */
    q->a[src] = malloc_min_heap_vtex(src, dist[src]);
    q->pos[src] = src;
    decrease_key(q, src, dist[src]);
    q->cur_sz = n_vtex;
    while(!is_empty(q))
    {/* q contains all nodes whose shortest distance is not yet found */
        min_heap_vtex * min_vtex = extract_min(q);
        int u = min_vtex->v; 
        for(vtex * i = g->adj[u].head; i; i = i->next)
        {
            int v = i->dest;
            if(is_in_min_heap(q, v) && dist[u] != ((unsigned)1 << 31) - 1
            && i->weight + dist[u] < dist[v])
            {
